...
{
    public class
        GoogleTranslateMeanFinder : AbstractMeanFinder<IGoogleTranslatorConfiguration, GoogleTranslateMeanOrganizer>
    {
        ...
        protected override async Task<TranslateResult> Find(TranslateRequest translateRequest)
        {
            ...
            if (response.Ok()) mean = await MeanOrganizer.OrganizeMean(response.Content);
